...
describe('Github demo oauth2 app', () => {
  before(() => {
  ...
  it('generates an authorize URL', () => {
    const bundle = {
      ...
      inputData: {
        state: '4444',
        redirect_uri: 'https://zapier.com/dashboard/auth/oauth/return/App5830CLIAPI/'
      },
      environment: {
    ...
    return appTester(App.authentication.oauth2Config.authorizeUrl, bundle)
      .then((authorizeUrl) => {
        authorizeUrl.should.eql('https://github.com/login/oauth/authorize?client_id=1234&state=4444&redirect_uri=https%3A%2F%2Fzapier.com%2Fdashboard%2Fauth%2Foauth%2Freturn%2FApp5830CLIAPI%2F&response_type=code');
      });
  });
  ...
  it('includes the access token in future requests', () => {
    ...
    return appTester(App.authentication.test, bundle)
      .then((result) => {
        ...
        result.username.should.eql('rebecca-ghazali');
      });
  });
});
